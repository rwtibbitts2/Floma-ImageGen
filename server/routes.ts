...
// Note: Old /api/prompts routes removed - use /api/system-prompts instead
...
router.post('/intelligent-refine', requireAuth, async (req, res) => {
  try {
    const schema = z.object({
      ...
      testConcepts: z.array(z.any()).optional(),
    });
...
TEST CONCEPTS GENERATED BY CONCEPT PROMPT:
${testConcepts && testConcepts.length > 0 
  ? testConcepts.map((concept, idx) => `${idx + 1}. ${typeof concept === 'string' ? concept : JSON.stringify(concept)}`).join('\n')
  : 'No test concepts available'}

Please analyze:
1. Visual differences between the reference and preview images
2. Quality and alignment of the test concepts generated by the concept prompt
3. Whether test concepts match the reference image's tone, brand, and conceptual direction
