...
-   **Intelligent Refine**: An automated feedback loop using GPT-5 vision to compare the reference image with generated preview images and evaluate test concepts. When activated, GPT-5 performs dual analysis: (1) visual differences between reference and preview images, and (2) quality assessment of test concepts generated by the concept prompt. The AI scores each concept for brand/subject relevance (1-10), completeness, conflicts, and redundancy, then automatically generates refined versions of all three prompts (Style, Composition, Concept) plus their frameworks. Results include detailed test concept feedback with alignment scores and required fixes, plus an overall concept guidance summary. Users can accept or reject the AI-suggested refinements with a single click. Falls back to GPT-4o if GPT-5 is unavailable. All refinements are saved to history for undo functionality.
