...
-   **Intelligent Refine**: An automated feedback loop using GPT-5 vision to compare the reference image with generated preview images. When activated, GPT-5 analyzes visual differences and automatically generates refined versions of all three prompts (Style, Composition, Concept) plus their frameworks, with explanations of what changed and why. Users can accept or reject the AI-suggested refinements with a single click. Falls back to GPT-4o if GPT-5 is unavailable. All refinements are saved to history for undo functionality.
...
-   **Componentized System Prompts Architecture**: Migrated from hardcoded prompts to a database-backed system with admin management, allowing rapid iteration on extraction quality and A/B testing different prompt strategies. This includes a `concept_output_schema` for consistent, structured concept output and an `intelligent_refine` system prompt for multimodal refinement analysis.
